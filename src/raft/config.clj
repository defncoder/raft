...
(defn get-current-env-config
  ...
  (l/info "Env is: " (-> (System/getProperties)
                         (get "app.env")))
  
  (let [default-config (read-config "/resources/app.default.edn")]
    (if-let [envname (-> (System/getProperties)
                         (get "app.env"))]
      (merge default-config (read-config (str "/resources/app." envname ".edn")))
      default-config)))
